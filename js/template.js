...
! function(e, t) {
    ...
    } - 1 == location.href.indexOf("/book"), $(".open-menu").on("click", function(t) {
        t.preventDefault(), e && e.toggle()
    }), $("#menu a:not(.dropdown-toggle)").on("click", function() {
        e && e.close()
    }), $(".jumbotron").size() > 0 && $(".main-photo").css("height", $(window).height() + "px"), detectmob() === !1 || navigator.userAgent.match(/Windows Phone/i) ? $("input.date").pickmeup({
        hide_on_select: !0,
        format: "d b Y",
        min: new Date,
        change: function(e) {
